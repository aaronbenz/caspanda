...
from caspanda.metabear import ColumnMeta
from caspanda.metabear import KeyspaceMeta
from caspanda.metabear import TableMeta
...
class CasPanda(Cluster):
    ...
    def panda_factory(self, colnames, rows):
        ...
        """
        if len(rows) == 0:
            cf = CassandraFrame(session=self.session)
        else:
            cf = CassandraFrame(rows, columns=colnames, session=self.session)
